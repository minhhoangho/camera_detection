...
def make_valid_xml_name(key, attr):
    ...
    LOG.info(f'Inside make_valid_xml_name(). Testing key "{unicode_me(key)}" with attr "{unicode_me(attr)}"')
    key = escape_xml(key)
...
def convert(obj, ids, attr_type, item_func, cdata, parent="root"):
    ...
    LOG.info(f'Inside convert(). obj type is: "{type(obj).__name__}", obj="{unicode_me(obj)}"')
...
def convert_dict(obj, ids, parent, attr_type, item_func, cdata):
    ...
    LOG.info(f'Inside convert_dict(): obj type is: "{type(obj).__name__}", obj="{unicode_me(obj)}"')
    output = []
...
def convert_kv(key, val, attr_type, attr={}, cdata=False):
    ...
    return f"<{key}{attrstring}>{wrap_cdata(val) if cdata == True else escape_xml(val)}</{key}>"
...
def convert_bool(key, val, attr_type, attr={}, cdata=False):
    ...
    if attr_type:
        attr["type"] = get_xml_type(val)
    attrstring = make_attrstring(attr)
    return f"<{key}{attrstring}>{unicode(val).lower()}</{key}>"
...
def dicttoxml(obj, root=True, custom_root="root", ids=False, attr_type=True, item_func=default_item_func, cdata=False):
    ...
    - cdata specifies whether string values should be wrapped in CDATA sections.
      Default is False
    """
    LOG.info(f'Inside dicttoxml(): type(obj) is: "{type(obj).__name__}", obj="{unicode_me(obj)}"')
    output = []
    addline = output.append
    if root == True:
